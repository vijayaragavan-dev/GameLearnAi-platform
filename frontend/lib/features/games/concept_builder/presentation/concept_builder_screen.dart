...
class _ConceptBuilderScreenState extends ConsumerState<ConceptBuilderScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final identity = GameVisualRegistry.of(GameType.conceptBuilder);
      ...
      body: Stack(
        children: [
          SafeArea(
            child: Column(
              children: [
                // HUD — shared premium component with concept builder identity
                GameHud(
                  score: _score,
                  progress: progress,
                  progressLabel: 'BUILD ${_index + 1} / ${_challenges.length}',
                  timeRemaining: _fmt(_timer.remaining),
                  combo: _combo,
                  difficultyLabel: _difficulty.displayName,
                  accent: identity.accent,
                  gameIcon: identity.icon,
                  gameTitle: GameType.conceptBuilder.displayName,
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Spacer(),
                      Text(ch.difficulty.displayName.toUpperCase(), style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 1, color: AppColors.textSecondary)),
                    ],
                  ),
                ),
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // BUILD AREA — premium challenge surface with identity accent
                        GameChallengeSurface(
                          accent: identity.accent,
                          title: 'BUILD AREA (${_selectedIds.length}/${ch.correctOrder.length})',
                          icon: identity.icon,
                          child: Container(
                        ...
                        if (_showResult) ...[
                          ...
                          GameFeedbackSurface(
                            ...
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(_wasCorrect ? 'BUILD COMPLETE ✓' : 'BUILD INCORRECT', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: _wasCorrect ? AppColors.success : AppColors.error)),
